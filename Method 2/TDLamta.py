...
def updateQ(play, Qmat, reward, alpha=0.1, discount=0.9):
    ...
    lastAction = play.loc[play.index[- 1], 'yAction']
    ...
    for k in range(1, T):
        ...
        q = discount ** (T - k -1) * Qmat.loc[int(float(lastState)), lastAction]
        ...
        for j in range (k, play.shape[0]):
            ...
            q += reward.loc[int(float(prevState)), (preAction, int(float(currState)))] * (discount ** (j-1))


        Qmat.loc[int(float(prevState)), preAction] = (1-alpha)* Qmat.loc[int(float(prevState)), preAction] + alpha * (1-discount)*q
