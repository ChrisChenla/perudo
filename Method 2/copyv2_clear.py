...
def liarsDiceRound(players, control, playerDiceCount, agents, gameStates, reward, Qmat, a = 1, verbose = 0):
    ...
    while (not Called):
        ...
        if (playerDiceCount[control]) > 0:
            ...
            if (action == "Call"):
                if (verbose > 0):
                    print('Player {} called and the roll table is {}'.format(control,rollTable))
                if ((diceQuantity) > (rollTable.iloc[0, diceValue-1])):
                    penalty[prev] = penalty[prev] - 1
                    if(verbose > 0):
                        print("Player {} lost a die 1 \n".format(prev))

                else:
                    penalty[control] = penalty[control] - 1
                    if (verbose > 0):
                        print("Player {} lost a die 2 \n".format(prev))
...
def updateSarsa (play, Qmat, reward, alpha=0.1, discount=0.9):
    ...
    for k in range(1, play.shape[0]):
        ...
        Qmat.loc[int(float(prevState)), action] = (1 - alpha) * Qmat.loc[int(float(prevState)), action] + alpha * (
                    reward.loc[int(float(prevState)), (action, int(float(currState)))] + discount * (
                Qmat.loc[int(float(currState)),action]))
