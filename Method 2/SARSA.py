...
def updateQ(play, Qmat, reward, alpha=0.1, discount=0.9):
    ...
    for k in range(1, play.shape[0]):
        ...
        # SARSA
        Qmat.loc[int(float(prevState)), prevAction] = (1 - alpha) * Qmat.loc[int(float(prevState)), prevAction] + alpha * (
                    reward.loc[int(float(prevState)), (prevAction, int(float(currState)))] + discount * (
