...
def buildAgent(bluffProb, method="random"):
    def buildAgentFn(pars, Qmat_1,Qmat_2):
...
def liarsDiceRound(players, control, playerDiceCount, agents, gameStates, reward, Qmat_1,Qmat_2, a=1, verbose=0):
    rolls = []
    ...
    reslist = agents[control](pars, Qmat_1,Qmat_2)
    ...
    if (verbose > 0):
        print(
            "dice value {} ; dice quantity {} the control right now is {}  \n".format(diceValue, diceQuantity, control))
    ...
    while (not Called):
        ...
        if (playerDiceCount[control]) > 0:
            ...
            agentActionReslist = agents[control](pars=pars, Qmat_1=Qmat_1,Qmat_2=Qmat_2)
...
def updateQ(play, Qmat_1, Qmat_2, reward, alpha=0.2, discount=0.8):
    ...
    for k in range(1, T):
        ...
        # valueEachRound.append(q)

    return Qmat_1, Qmat_2, sum(valueEachRound)


def playLiarsDice(agents, players=4, numDice=6, auto=True, Qmat_1=np.array([]), Qmat_2=np.array([]),train=True, printTrans=False):
    ...
    if (Qmat_1.size == 0):
        Qmat_1 = pd.DataFrame(np.zeros((reward.shape[0], 2)), columns=['Raise', 'Call'])
        Qmat_2 = pd.DataFrame(np.zeros((reward.shape[0], 2)), columns=['Raise', 'Call'])
        # Emat = pd.DataFrame(np.zeros((reward.shape[0], 2)), columns=['Raise', 'Call'])
    ...
    while (playersLeft > 1):
        results = liarsDiceRound(players=players, control=ctrl, playerDiceCount=ndice, gameStates=gameStates,
                                 reward=reward, Qmat_1=Qmat_1,Qmat_2=Qmat_2, agents=agents, a=not (auto))
        ...
        if (train):
            Qmat_1,Qmat_2, valueEachRound = updateQ(results[1], Qmat_1,Qmat_2, reward)
    ...
    return [np.where(ndice > 0)[0], Qmat_1,Qmat_2]
