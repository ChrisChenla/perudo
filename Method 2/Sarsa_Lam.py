...
def liarsDiceRound(players, control, playerDiceCount, agents, gameStates, reward, Qmat, Emat,a=1, verbose=0):
    rolls = []
...
def updateQ(play, Qmat, Emat,reward, alpha=0.1, discount=0.9,trace_decay=0.9):
    valueEachRound = []
    ...
    for k in range(1, play.shape[0]):
        ...
        Emat.loc[int(float(prevState)), preAction] = Emat.loc[int(float(prevState)), preAction] + 1
    ...
    return Qmat, Emat, sum(valueEachRound)



def playLiarsDice(agents, players=4, numDice=6, auto=True, Qmat=np.array([]), Emat=np.array([]),train=True, printTrans=False):
    ...
    while (playersLeft > 1):
        results = liarsDiceRound(players=players, control=ctrl, playerDiceCount=ndice, gameStates=gameStates,
                                 reward=reward, Qmat=Qmat,Emat = Emat, agents=agents, a=not (auto))
        ...
        if (train):
            Qmat, Emat, valueEachRound = updateQ(results[1], Qmat,Emat, reward)
...
for k in tqdm(range(its)):
    out = playLiarsDice(agents=agents, players=len(agents), numDice=3, Qmat=Qmat, Emat=Emat,printTrans=False)
    winners[k] = out[0]
    # 21 29
    Qmat = out[1]
    Emat = np.array([])
