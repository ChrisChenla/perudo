...
def liarsDiceRound(players, control, playerDiceCount, agents, gameStates, reward, Qmat, a=1, verbose=2):
    rolls = []
    ...
    if (verbose > 0):
        print('place first bid\n Player {} has control\n'.format(control))

    p0State = np.array(gameStates[(gameStates.loc[:, 'Total'] == totalDice) & (
            gameStates.loc[:, 'p0'] == playerDiceCount[0]) & (
                                          gameStates.loc[:, 'prob_cat'] == totalDice)].index.tolist())
    ...
    p0Action = "Raise"
    if (verbose > 0):
        print("the initial state here is {} by {} ".format(p0State, control))
    ...
    if (verbose > 0):
        print("dice value {} ; dice quantity {} the control right now is {}  \n".format(diceValue, diceQuantity, control))
            ...
            #
                ...
                p0State = np.array(gameStates[(gameStates.loc[:, 'Total'] == totalDice) & (
                            gameStates.loc[:, 'p0'] == playerDiceCount[0]) & (
                                          gameStates.loc[:, 'prob_cat'] == probCat)].index.tolist())
                if (verbose > 0):
                    print('dice value {} ;  dice quantity {}'.format(diceValue, diceQuantity))
...
#                                           gameStates.loc[:, 'prob_cat'] == probCat)].index.tolist())
#     p0Action = "Raise"
#     print("the initial state S here is {} of {} ".format(p0State,control))
#
#     yCtrl = np.array([])
#     yState = np.array([])
#     yAction = np.array([])
...
#         yAction = np.append(yAction, p0Action)
#
#         print('Before going to the loop which is control {} state S {} and action A {}'.format(yCtrl,yState,yAction))
#
#     Called = False
#     while (not Called):
#         # if (action % in% c("call", "c"))
#
#         if (action == "Call"):
#             print("the a' is {}".format(action))
#
#             # if (verbose > 0):
#             #     print('Player {} called and the roll table is\n {}'.format(control, rollTable))
#                 # print("dice value {} ; dice quantity {} \n".format(diceValue, diceQuantity))
#
#             if ((diceQuantity) > (rollTable.iloc[0, diceValue - 1])):
#                 penalty[prev] = penalty[prev] - 1
#                 # if (verbose > 0):
...
#             # yAction = np.append(yAction, p0Action)
#
#             # print("after call is state S' {} ".format(p0State))
#             # print("Y ctrl {} and y state {} y action is {} ".format(yCtrl,yState,yAction))
#
#             # probCat = calcProb(np.array([totalDice, playerDiceCount[0], diceQuantity, playerDiceQty, diceValue]))
#             # p0State = np.array(gameStates[(gameStates.loc[:, 'Total'] == totalDice - 1) & (
#             #         gameStates.loc[:, 'p0'] == playerDiceCount[0] + penalty[0]) & (
...
#         else:
#             # if (verbose > 0):
#             #     print('Player {} raise'.format(control))
#             print("the a' is Raise")
#             diceValue = agentActionReslist[1]
#             diceQuantity = agentActionReslist[2]
#
#             probCat = calcProb(np.array([totalDice, playerDiceCount[0], diceQuantity, playerDiceQty, diceValue]))
...
#                                                   gameStates.loc[:, 'prob_cat'] == probCat)].index.tolist())
#
#             print("the S' value after raising is {}".format(p0State))
#
#             # if (verbose > 0):
#             #     print('dice value {} ;  dice quantity {}'.format(diceValue, diceQuantity))
#
...
#         # if (control == 0) and yAction[-1] == 'Raise':
#         #
#         #     yCtrl = np.append(yCtrl, control)
#         #     yState = np.append(yState, p0State)
#         #     yAction = np.append(yAction, p0Action)
...
#     # play = pd.DataFrame(np.hstack((yCtrl[:, None], yState[:, None], yAction[:, None])),
#     #                     columns=['yCtrl', 'yState', 'yAction'])
#     # print('HERE THE PLAY AFETER ROUND IS' )
#     # print(play)
#     return [penalty, play]
    ...
    def buildAgentFn(pars, Qmat):
        ...
        if (bluff):
            ...
            if (newDiceQuantity > pars[1]):
                ...
                newDiceValue = pars[2] + 1
                # print('return call for value {} type is {} and quantity {}'.format(newDiceValue,type(newDiceValue), newDiceQuantity))
                if (newDiceValue == 7):
                    # print('return call for value {} and quantity {}'.format(newDiceValue,newDiceQuantity))
                    return ["Call", pars[2], pars[3]]
        ...
        # print('Now final raise is is {} and {}'.format(newDiceValue,newDiceQuantity))
...
def updateQ(play, Qmat, reward, alpha=0.1, discount=0.9):
    ...
    for k in range(1, play.shape[0]):
        ...
        prevAction = play.loc[play.index[k - 1], 'yAction']

        # Qmat.loc[int(float(prevState)), action] = (1 - alpha) * Qmat.loc[int(float(prevState)), action] + alpha * (
        #             reward.loc[int(float(prevState)), (action, int(float(currState)))] + discount * max(
        #         Qmat.loc[int(float(currState)),]))

        # SARSA
        Qmat.loc[int(float(prevState)), prevAction] = (1 - alpha) * Qmat.loc[
            int(float(prevState)), prevAction] + alpha * (reward.loc[int(float(prevState)), (
                                                              prevAction, int(float(currState)))] + discount * (
                                                                  Qmat.loc[int(float(currState)), currAction]))
...
def playLiarsDice(agents, players=4, numDice=6, auto=True, Qmat=np.array([]), train=True, printTrans=False):
    ...
    while (playersLeft > 1):
        results = liarsDiceRound(players=players, control=ctrl, playerDiceCount=ndice, gameStates=gameStates,
                                 reward=reward, Qmat=Qmat, agents=agents, a=not (auto))
...
for k in tqdm(range(its)):
    out = playLiarsDice(agents=agents, players=len(agents), numDice=4, Qmat=Qmat, printTrans=False)
    winners[k] = out[0]
    # 21 29
    Qmat = out[1]
    if winners[k] == 0:
