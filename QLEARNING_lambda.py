    ...
    while (not Called):
        ...
        if (playerDiceCount[control]) > 0:
            ...
            if (control == 0 and (not (action is None))):
                ...
                probCat = calcProb(np.array([totalDice, playerDiceCount[0], diceQuantity, playerDiceQty, diceValue]))
                p0State = np.array(gameStates[(gameStates.loc[:, 'Total'] == totalDice) & ( gameStates.loc[:, 'p0'] == playerDiceCount[0]) & (
                   gameStates.loc[:, 'prob_cat'] == probCat)].index.tolist())
            ...
            if (action == "Call"):
                ...
                if (verbose > 0):
                    ...
                    print("dice value {} ; dice quantity {} for checking is \n".format(diceValue, diceQuantity))
...
def updateQ(play, Qmat, Emat,reward, alpha=0.1, discount=0.9,trace_decay=0.9):
    ...
    for k in range(1, play.shape[0]):
        ...
        delta = reward.loc[int(float(prevState)), (preAction, int(float(currState)))]+ discount * (max(Qmat.loc[int(float(currState)),])) - Qmat.loc[int(float(prevState)), preAction]
